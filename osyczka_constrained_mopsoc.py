...
def g5(x):
    a = make_a(x)
    return 4.0 - (a[2] - 3.0) ** 2.0 - a[3]
def g6(x):
    a = make_a(x)
    return (a[4] - 3.0) ** 2.0 + a[5] - 4.0
...
class PSOCategorical:
    def __init__(self, n_particles, scaling_factor, function1, function2):
        ...
        self.categories = [list(range(200)), list(range(200)), list(range(200)), list(range(200)), list(range(200)), list(range(200))]
        ...
        for particle in range(self.n_particles):
            for var in range(len(self.categories)):
                for prob in range(len(self.categories[var])):
                    ...
                    if self.positions_categorical[particle][var][prob] > 1.0:
                        self.positions_categorical[particle][var][prob] = 1.0
                    elif self.positions_categorical[particle][var][prob] < 0.0:
    ...
    def calculate_new_velocities(self, dyn_turb):
        for particle in range(self.n_particles):
            for var in range(len(self.categories)):
                for prob in range(len(self.categories[var])):
                    if random() < 1.0 - dyn_turb:
                        turbulence = random()
                    else:
                        turbulence = 0.0
    ...
    def run(self):
        ...
        for iteration in range(self.n_iterations):
            ...
            self.calculate_new_velocities(float(iteration) / float(self.n_iterations))
            ...
            self.fitness = Parallel(n_jobs=-2)(delayed(fitness_function)(sample, [self.function1, self.function2]) for sample in self.samples)
            self.fitness_and_samples = list(zip(self.fitness, self.samples))
            for item in self.fitness_and_samples:                
                if g1(item[1]) < 0 or g2(item[1]) < 0 or g3(item[1]) < 0 or g4(item[1]) < 0 or g5(item[1]) < 0 or g6(item[1]) < 0:
                    item[0][1] = 500.0
                    item[0][0] = 0.0
            ...
            with open("osyczka8_1.dat", "a") as term:
            ...
            if iteration % 5 == 0:
                ...
                with open("osyczka8_5.dat", "a") as term:
            ...
            if iteration % 10 == 0:
                ...
                with open("osyczka8_10.dat", "a") as term:
            ...
            with open("osyczka8_mopsoc.dat", "a") as out:
