...
def g1(x):
    a = [0.025 * x[0], 0.015 * x[1]]
    return 25.0 - ((a[0] - 5.0) ** 2.0 + a[1] ** 2.0)


def g2(x):
    a = [0.025 * x[0], 0.015 * x[1]]
    return (a[0] - 8.0) ** 2.0 + (a[1] + 3.0) ** 2.0 - 7.7
...
class PSOCategorical:
    def __init__(self, n_particles, scaling_factor, function1, function2):
        ...
        self.means = [0 for _ in range(self.n_functions)]
        self.std_devs = [0 for _ in range(self.n_functions)]

    def normalise_functions(self):
        self.fitness = [[(self.fitness[part][x] - self.means[x]) / self.std_devs[x] for x in range(self.n_functions)] for part in range(self.n_particles)]

    def denormalise_archive(self):
        for particle in self.archive:
            particle = [particle[0][x] * self.std_devs[x] + self.means[x] for x in range(self.n_functions)]


    def normalise_archive(self):
        for particle in self.archive:
            particle = [(particle[0][x] - self.means[x]) / self.std_devs[x] for x in range(self.n_functions)]

    def get_mean(self, new, iteration):
        self.means = [(self.means[x] * (iteration - 1) * self.n_particles + sum([new[i][x] for i in range(self.n_particles)])) / (self.n_particles * iteration) for x in range(self.n_functions)]

    def get_std_dev(self, new, iteration):
        to_sum = [self.std_devs[x] ** 2.0 * (self.n_particles * (iteration - 1) - 1) for x in range(self.n_functions)]
        self.std_devs = [sqrt((to_sum[x] + sum([(new[i][x] - self.means[x]) ** 2.0 for i in range(self.n_particles)])) / (self.n_particles * iteration - 1)) for x in range(self.n_functions)]
    ...
    def run(self):
        ...
        for iteration in range(1, self.n_iterations):
            ...
            self.denormalise_archive()
            self.get_mean(self.fitness, iteration)
            # print self.means, self.std_devs
            self.get_std_dev(self.fitness, iteration)
            self.normalise_archive()
            ...
            # with open("bihn2_1.dat", "a") as term:
            ...
            #     with open("bihn2_5.dat", "a") as term:
            ...
            #     with open("bihn2_10.dat", "a") as term:
            #         term.write("{} {}\n".format(consolidation_counter10, improvement_counter10))

            plt.cla()
            ax.scatter([item[0][0] * self.std_devs[0] + self.means[0] for item in self.archive], [item[0][1] * self.std_devs[1] + self.means[1] for item in self.archive])
            plt.pause(0.01)

            with open("bihn_normal.dat", "a") as out:
                for item in self.archive:
                    for fun in range(self.n_functions):
                        out.write("{} ".format(item[0][fun] * self.std_devs[fun] + self.means[fun]))
                    out.write("{}\n".format(item[1]))
                out.write("\n")
